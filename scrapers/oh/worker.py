...
log = logging.getLogger("oh")

logging.basicConfig(filename="ohscraper.log", filemode="a", encoding="utf-8", format="%(asctime)s %(levelname)s %(message)s")
...
class OshwPaginator:
    def __init__(self, ses: ThrottledHTTPSession):
        self.ses = ses
        self.next_page = 1
        self.parser = OshwPageParser()
    def url_gen(self, page_num):
        return f"https://oshwhub.com/explore?projectSort=updatedTime&page={page_num}"
    def recover_progress(self, last_estimated_id):
...
class OshwPipeline:
    def __init__(self):
        ...
        self.pages = OshwPaginator(self.ses)
        self.parser = OshwPageParser()
        self.db = OshwDatabase()
        self.finished = False
    ...
    def fetch_newest_project_covers(self, limit=100):
        self.finished = False
    ...
    def fetch_older_project_covers(self, limit=100):
        self.finished = False
        prev_next_page = self.pages.next_page
